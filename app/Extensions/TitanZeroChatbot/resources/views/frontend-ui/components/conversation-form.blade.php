...
>
    ...
    <div class="flex min-h-14 w-full items-center">
        <textarea
            ...
            @input.throttle.50ms="$el.scrollTop = $el.scrollHeight; $refs.sendBtn.classList.toggle('active', $el.value.trim() || pendingMedia); activeConversation && window.TitanChatbotLocalServices?.saveDraft(activeConversationData?.local_uuid || activeConversation, $el.value)"
            x-ref="message"
            data-chatbot-offline-draft
            :data-conversation-uuid="activeConversationData?.local_uuid || activeConversation"
        ></textarea>
