...
>
    ...
    <div class="flex flex-col gap-7 pt-9">
        <section class="rounded-2xl border border-heading-foreground/10 bg-heading-foreground/[0.025] p-4">
            <div class="mb-4 flex items-start justify-between gap-3">
                <div>
                    <h3 class="mb-1 text-sm font-semibold text-heading-foreground">@lang('Choose an app')</h3>
                    <p class="m-0 text-2xs/5 opacity-60">@lang('Start with a Titan app template, then customise its chatbot, channels and appearance.')</p>
                </div>
                <span class="rounded-full bg-heading-foreground/5 px-2 py-1 text-[10px] font-semibold" x-text="titanTemplates.length + ' apps'"></span>
            </div>

            <div class="grid grid-cols-2 gap-2 sm:grid-cols-3">
                <template x-for="template in titanTemplates" :key="template.slug">
                    <button
                        class="group relative min-h-24 rounded-xl border p-3 text-start transition hover:-translate-y-0.5 hover:border-heading-foreground/30 hover:shadow-sm"
                        :class="selectedTitanTemplate === template.slug ? 'border-heading-foreground bg-heading-foreground text-heading-background' : 'border-heading-foreground/10 bg-background'"
                        type="button"
                        @click="applyTitanTemplate(template)"
                    >
                        <span class="mb-2 block text-xl" x-text="template.icon"></span>
                        <span class="block text-xs font-semibold" x-text="template.name"></span>
                        <span class="mt-1 block text-[10px] leading-4 opacity-60" x-text="template.description"></span>
                        <span
                            class="absolute end-2 top-2 grid size-5 place-items-center rounded-full bg-background/15 text-[10px]"
                            x-show="selectedTitanTemplate === template.slug"
                        >✓</span>
                    </button>
                </template>
            </div>
        ...
        @if (class_exists(\App\Extensions\Chatbot\System\Helpers\ChatbotHelper::class) && \App\Extensions\Chatbot\System\Helpers\ChatbotHelper::planAllowsHumanAgent())
            @includeIf('chatbot::staff-inbox.partials.chatbot-config')
        @endif
		...
		@include('chatbot::home.edit-window.edit-steps.generative-ui-config')
