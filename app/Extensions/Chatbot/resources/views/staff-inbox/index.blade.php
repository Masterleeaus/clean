@php
    ...
    $customerTagRoutes = [
        'index' => $hasCustomerTagExtension ? route('dashboard.chatbot.inbox.customer-tags.index') : null,
        'store' => $hasCustomerTagExtension ? route('dashboard.chatbot.inbox.customer-tags.store') : null,
        'sync' => $hasCustomerTagExtension ? route('dashboard.chatbot.inbox.customer-tags.sync') : null,
    ];
...
@section('content')
    <nav class="flex items-center gap-2 border-b bg-background px-4 py-2" data-chatbot-workspace-nav>
        <button type="button" class="lqd-btn lqd-btn-primary" data-chatbot-workspace-tab="customer">{{ __('Customer Inbox') }}</button>
        <button type="button" class="lqd-btn lqd-btn-ghost" data-chatbot-workspace-tab="team">{{ __('Team Chat') }}</button>
        <button type="button" class="lqd-btn lqd-btn-ghost" data-chatbot-workspace-tab="channels">{{ __('Channels') }}</button>
        <span class="ms-auto text-xs opacity-60" data-chatbot-workspace-status>{{ __('Online') }}</span>
    </nav>
    <div
        id="chatbot-customer-inbox-workspace"
        data-chatbot-workspace="customer"
        class="lqd-ext-chatbot-history flex h-[calc(100vh-49px)] flex-col max-lg:[--header-height:65px] lg:flex-row"
        x-data="chatbotStaffInbox"
    >
        <div
        ...
        >
            @includeIf('chatbot::staff-inbox.partials.conversations-filter')
            ...
            >
                @includeIf('chatbot::staff-inbox.partials.conversations-sort')

                @includeIf('chatbot::staff-inbox.partials.conversations-channel-filter')

                @include('chatbot::staff-inbox.partials.conversations-list')
            </div>
        </div>
        ...
        >
            @include('chatbot::staff-inbox.partials.messages-header')
            ...
            >
                <div class="mt-auto space-y-2 px-4 max-lg:pb-[calc(var(--bottom-menu-height)+2rem)] xl:px-10">
                    @include('chatbot::staff-inbox.partials.messages-list')
                </div>

                @include('chatbot::staff-inbox.partials.messages-form')
            </div>
        </div>
        ...
        >
            @include('chatbot::staff-inbox.partials.contact-info-head')

            <div class="grid grow grid-cols-1 place-items-start overflow-y-auto">
                @include('chatbot::staff-inbox.partials.contact-info-tab-details', ['hasCustomerTagExtension' => $hasCustomerTagExtension])
                @include('chatbot::staff-inbox.partials.contact-info-tab-history')
            </div>
        </div>
    </div>
@endsection
            ...
            document.addEventListener('alpine:init', () => {
                Alpine.data('chatbotStaffInbox', () => ({
                    ...
                    async init() {
                        ...
                        Alpine.store('chatbotStaffInbox', this);

                        this.initEmojiPicker();
                        this.initDateRangePicker();
                        this.$refs.message?.addEventListener('input', () => {
                            clearTimeout(this._draftTimer);
                            this._draftTimer = setTimeout(() => this.saveActiveDraft(), 350);
                        });
                    },
                    async loadLocalChats(query = '') {
                        try {
                            if (!window.TitanChatbotStaffLocalFirst) return;
                            const cached = await window.TitanChatbotStaffLocalFirst.loadConversations({ query });
                            if (cached.length) {
                                this.chatsList = cached.map(chat => this.normalizeConversationPayload(chat));
                                this.activeChat = this.activeChat || this.chatsList[0];
                                if (this.activeChat) {
                                    this.activeChat.histories = await window.TitanChatbotStaffLocalFirst.loadMessages(this.activeChat);
                                    await this.restoreActiveDraft();
                                }
                            }
                        } catch (error) {
                            console.warn('[Chatbot staff inbox] Local cache unavailable.', error);
                        }
                    },
                    async restoreActiveDraft() {
                        if (!this.activeChat || !window.TitanChatbotStaffLocalFirst || !this.$refs.message) return;
                        const draft = await window.TitanChatbotStaffLocalFirst.loadDraft(this.activeChat);
                        const body = draft?.payload?.body || draft?.body || '';
                        if (body && !this.$refs.message.value) this.$refs.message.value = body;
                    },
                    async saveActiveDraft() {
                        if (!this.activeChat || !window.TitanChatbotStaffLocalFirst || !this.$refs.message) return;
                        await window.TitanChatbotStaffLocalFirst.saveDraft(this.activeChat, this.$refs.message.value, {
                            is_internal_note: this.$refs.isInternalNote?.value === '1'
                        });
                    },
                    async searchLocalWorkspace(query) {
                        if (!window.TitanChatbotStaffLocalFirst) return null;
                        return window.TitanChatbotStaffLocalFirst.search(query);
                    },
                    async editLocalMessage(message) {
                        if (!message?.local_uuid) return;
                        const next = prompt('{{ __('Edit queued message') }}', message.message || '');
                        if (next === null) return;
                        const updated = await window.TitanChatbotStaffLocalFirst.editMessage(message.local_uuid, { message: next, content: next });
                        Object.assign(message, updated, { message: next, content: next });
                    },
                    async cancelLocalMessage(message) {
                        if (!message?.local_uuid) return;
                        const updated = await window.TitanChatbotStaffLocalFirst.cancelMessage(message.local_uuid);
                        Object.assign(message, updated, { sync_status: 'cancelled' });
                    },
                    async retryLocalMessage(message) {
                        if (!message?.local_uuid) return;
                        const updated = await window.TitanChatbotStaffLocalFirst.retryMessage(message.local_uuid);
                        Object.assign(message, updated, { sync_status: 'queued' });
                    },
                    initDateRangePicker() {
                        this.$nextTick(() => {
                    ...
                    async updateConversationDetails({
                        ...
                        @else
                            ...
                            const res = await fetch('{{ route('dashboard.chatbot.inbox.conversations.update') }}', {
                                method: 'PUT',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
                        ...
                        @if (setting('ably_public_key') && is_string(setting('ably_public_key')) && strlen(setting('ably_public_key')) > 8)
                            ...
                            await channel.subscribe("conversation", async message => {
                                ...
                                if (conversationIsActive) {
                                    ...
                                    fetch('{{ route('dashboard.chatbot.inbox.history') }}?conversation_id=' + this.activeChat.id, {
                                        method: 'GET',
                                        headers: {
                                            'Accept': 'application/json'
                                        },
                    ...
                    async onSendMessage() {
                        ...
                        @else
                            ...
                            const localMessage = await window.TitanChatbotStaffLocalFirst?.queueMessage(this.activeChat, newUserMessage);
                            if (localMessage) Object.assign(newUserMessage, localMessage);
                            ...
                            const sendFirstMessage = async () => {
                                ...
                                const res = await fetch('{{ route('dashboard.chatbot.inbox.history') }}', {
                                    method: 'POST',
                                    headers: {
                                        'Accept': 'application/json',
                                    },
                    ...
                    async fetchCannedResponses() {
                        ...
                        try {
                            const res = await fetch('{{ route('dashboard.chatbot.inbox.canned-responses') }}', {
                                method: 'GET',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
                    ...
                    async rewriteMessage(variantKey) {
                        ...
                        @else
                            ...
                            try {
                                const res = await fetch('{{ route('dashboard.chatbot.inbox.message-rewrite') }}', {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/json',
                                        'Accept': 'application/json',
                    ...
                    async fetchHistories(id) {
                        const conversation = this.chatsList.find(chat => String(chat.id) === String(id)) || this.activeChat || { id };
                        let cached = [];
                        try {
                            cached = await window.TitanChatbotStaffLocalFirst?.loadMessages(conversation) || [];
                        } catch (_) {}

                        try {
                            const res = await fetch('{{ route('dashboard.chatbot.inbox.history') }}?conversation_id=' + id, {
                                method: 'GET',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
                            ...
                            await window.TitanChatbotStaffLocalFirst?.cacheMessages(conversation, data.data || []);
                            return await window.TitanChatbotStaffLocalFirst?.loadMessages(conversation) || data.data || cached;
                        } catch (error) {
                    ...
                    async fetchChats(opts = {}) {
                        ...
                        await window.TitanChatbotStaffLocalFirst?.cacheConversations(conversations);
                        conversations = await window.TitanChatbotStaffLocalFirst?.loadConversations({ limit: 200 }) || conversations;
                    ...
                    async handleConversationsSearch() {
                        ...
                        this.fetching = true;
                        const localResults = await window.TitanChatbotStaffLocalFirst?.loadConversations({ query: query || '', limit: 100 }) || [];
                        if (!navigator.onLine) {
                            this.chatsList = localResults;
                            this.activeChat = localResults[0] || null;
                            this.fetching = false;
                            return;
                        }

                        const res = await fetch('{{ route('dashboard.chatbot.inbox.conversations.search') }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',
                    ...
                    async handleMessagesSearch(inputElement) {
                        ...
                        await window.TitanChatbotStaffLocalFirst?.markRead(this.activeChat);
                        await this.restoreActiveDraft();
                            }
                            return;
                        }
                    ...
                    async closeConversation(conversationId) {
                        ...
                        const res = await fetch('{{ route('dashboard.chatbot.inbox.conversations.closed') }}?conversation_id=' + conversationId, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',
                    ...
                    async pinConversation(conversationId) {
                        ...
                        const res = await fetch('{{ route('dashboard.chatbot.inbox.conversations.pinned') }}?conversation_id=' + conversationId, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',
                    ...
                    async getConversationsHistory(sessionId) {
                        ...
                        try {
                            const res = await fetch(
                                `{{ route('dashboard.chatbot.inbox.conversations.history.session') }}?sessionId=${sessionId}`, {
                                    method: 'GET',
                                    headers: {
                                        'Content-Type': 'application/json',
                                        'Accept': 'application/json',
                    ...
                    async deleteConversation(conversationId) {
                        ...
                        @else
                            ...
                            const res = await fetch('{{ route('dashboard.chatbot.inbox.destroy') }}', {
                                method: 'DELETE',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
...
@include('chatbot::staff-inbox.partials.offline-runtime')

@include('chatbot::staff-inbox.team-chat.panel')
