            ...
            >
                ...
                <div class="lqd-ext-chatbot-window-conversation-message-content-wrap">
                    <div
                        ...
                        @include('chatbot::frontend-ui.components.generative-ui-message')
                        ...
                        <template x-if="message.role === 'user' && message.sync_status && message.sync_status !== 'synced'">
                            <div class="mt-1 flex items-center gap-2 text-[10px] opacity-70" :data-sync-status="message.sync_status">
                                <span x-text="({ 'local-only': 'On device', queued: 'Queued', sending: 'Sending', failed: 'Failed', 'needs-review': 'Needs review', conflict: 'Conflict', cancelled: 'Cancelled' })[message.sync_status] || message.sync_status"></span>
                                <button type="button" x-show="['failed', 'needs-review', 'conflict'].includes(message.sync_status)" data-chatbot-local-action="retry" :data-local-uuid="message.local_uuid" class="underline">{{ __('Retry') }}</button>
                                <button type="button" x-show="['queued', 'failed', 'needs-review'].includes(message.sync_status)" data-chatbot-local-action="edit" :data-local-uuid="message.local_uuid" class="underline">{{ __('Edit') }}</button>
                                <button type="button" x-show="['queued', 'failed', 'needs-review'].includes(message.sync_status)" data-chatbot-local-action="cancel" :data-local-uuid="message.local_uuid" class="underline">{{ __('Cancel') }}</button>
                            </div>
                        </template>
