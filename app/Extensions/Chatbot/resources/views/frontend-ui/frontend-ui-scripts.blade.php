    ...
    (() => {
        document.addEventListener('alpine:init', () => {
            Alpine.data('externalChatbot', (isEditor = false) => ({
                ...
                titanDrawerOpen: false,
                titanSettingsOpen: false,
                titanActiveSetting: null,
                titanActiveSettingLabel: '',
                titanSettings: window.TitanSettings?.get?.() || {},
                titanRuntimeStatus: window.TitanSettings?.status?.() || {},
                titanProvider: { id: 'openai', api_key: '', model: '', base_url: '' },
                titanVaultPassphrase: '',
                titanSettingMessage: '',
                titanDiagnosticSummary: '',
                titanCapabilities: window.TitanWorkCore?.device?.capabilities || {},
                titanView: @json($titanShell['default_view'] ?? 'home'),
                titanNavigation: @json($titanShell),
                titanBadges: {},
                titanConnectionState: navigator.onLine ? 'online' : 'offline',

                init() {
                    this.windowState = this.$el.getAttribute('data-window-state');
                    window.addEventListener('online', () => this.titanConnectionState = 'online');
                    window.addEventListener('offline', () => this.titanConnectionState = 'offline');
                    window.addEventListener('workcore:sync-started', () => this.titanConnectionState = 'syncing');
                    window.addEventListener('workcore:sync-completed', () => this.titanConnectionState = navigator.onLine ? 'online' : 'offline');
                    window.addEventListener('titan:prompt-requested', (event) => { const prompt = event.detail?.prompt; if (!prompt) return; this.openTitanChat(); this.$nextTick(() => this.sendPromptMessage(prompt)); });
                    ...
                    window.addEventListener('chatbot:edit-local-message-requested', async (event) => {
                        const record = await window.TitanChatbotRepositories?.messages.findByLocalUuid(event.detail?.localUuid);
                        if (!record || !this.$refs.message) return;
                        this.$refs.message.value = record.payload?.message || record.payload?.content || '';
                        this.$refs.message.focus();
                        this.$refs.sendBtn?.classList.add('active');
                        this.$refs.message.dataset.editingLocalUuid = record.local_uuid;
                    });
                    window.addEventListener('chatbot:local-message-cancelled', (event) => {
                        this.messages = this.messages.filter(message => message.local_uuid !== event.detail?.message?.local_uuid);
                    });
                    window.addEventListener('chatbot:local-message-retried', (event) => {
                        const target = this.messages.find(message => message.local_uuid === event.detail?.message?.local_uuid);
                        if (target) target.sync_status = 'queued';
                    });

                    if (!window.__titanGenerativeUiIntentBridge) {
                        window.__titanGenerativeUiIntentBridge = true;
                        window.addEventListener('titan-generative-ui-action', (event) => {
                            const detail = { ...(event.detail || {}), authority: 'presentation-only' };
                            window.dispatchEvent(new CustomEvent('chatbot:generative-ui-intent', { detail }));
                            if (this.parentOrigin && window.parent !== window) {
                                window.parent.postMessage({ type: 'titan-generative-ui-intent', detail }, this.parentOrigin);
                            }
                        });
                    }
                ...
                connectToAgent() {
                    @if (isset($chatbot) && isset($session) && true)
                ...
                openTitanDrawer() { this.titanSettingsOpen = false; this.titanDrawerOpen = true; this.$nextTick(() => this.$refs.titanDrawerPanel?.focus()); },
                closeTitanDrawer() { this.titanDrawerOpen = false; this.$nextTick(() => this.$refs.titanDrawerTrigger?.focus()); },
                openTitanSettings() { this.titanDrawerOpen = false; this.titanSettingsOpen = true; this.$nextTick(() => this.$refs.titanSettingsPanel?.focus()); },
                closeTitanSettings() { this.titanSettingsOpen = false; this.titanActiveSetting = null; this.$nextTick(() => this.$refs.titanSettingsTrigger?.focus()); },
                openTitanSetting(setting, label = '') { this.titanActiveSetting = setting; this.titanActiveSettingLabel = label; this.refreshTitanSettingsStatus(); window.dispatchEvent(new CustomEvent('titan:setting-opened', { detail: { setting } })); },
                refreshTitanSettingsStatus() { this.titanSettings = window.TitanSettings?.get?.() || this.titanSettings; this.titanRuntimeStatus = window.TitanSettings?.status?.() || {}; this.titanCapabilities = window.TitanWorkCore?.device?.capabilities || {}; },
                saveTitanSettings() { window.TitanSettings?.update?.(this.titanSettings); this.refreshTitanSettingsStatus(); document.documentElement.dataset.titanTheme = this.titanSettings.theme || 'system'; document.documentElement.dataset.titanTextSize = this.titanSettings.text_size || 'normal'; },
                async saveTitanProvider() { try { await window.TitanSettings.saveProvider(this.titanProvider.id, this.titanProvider); this.titanProvider.api_key = ''; this.titanSettingMessage = 'Provider connected. Key encrypted in the device vault.'; this.refreshTitanSettingsStatus(); } catch (error) { this.titanSettingMessage = error.message; } },
                async unlockTitanVault() { try { await window.TitanWorkCore?.vault?.unlock?.(this.titanVaultPassphrase); this.titanVaultPassphrase = ''; this.titanSettingMessage = 'Device vault unlocked.'; this.refreshTitanSettingsStatus(); } catch (error) { this.titanSettingMessage = error.message; } },
                lockTitanVault() { window.TitanWorkCore?.vault?.lock?.(); this.refreshTitanSettingsStatus(); },
                async syncTitanNow() { this.titanConnectionState = 'syncing'; try { await window.TitanWorkCore?.client?.sync?.(); } finally { this.titanConnectionState = navigator.onLine ? 'online' : 'offline'; this.refreshTitanSettingsStatus(); } },
                openTitanQueue() { this.closeTitanSettings(); this.navigateTitanView('queue'); window.dispatchEvent(new CustomEvent('workcore:open-queue')); },
                async testTitanWorkCore() { try { const result = await window.TitanWorkCore?.client?.diagnostics?.(); this.titanDiagnosticSummary = JSON.stringify(result || { connected: Boolean(window.TitanWorkCore?.client) }, null, 2); } catch (error) { this.titanDiagnosticSummary = error.message; } },
                async runTitanDiagnostics() { const result = await window.TitanSettings?.diagnostics?.(); this.titanDiagnosticSummary = JSON.stringify(result || {}, null, 2); },
                async requestTitanNotifications() { if (!('Notification' in window)) { this.titanSettingMessage = 'Notifications are unavailable on this device.'; return; } this.titanSettingMessage = `Notification permission: ${await Notification.requestPermission()}`; },
                exportTitanSettings() { const blob = new Blob([JSON.stringify(window.TitanSettings?.get?.() || {}, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = 'titan-settings.json'; link.click(); URL.revokeObjectURL(url); },
                navigateTitanView(view) {
                    this.titanView = view;
                    window.dispatchEvent(new CustomEvent('titan:navigate', { detail: { view, template: this.titanNavigation?.slug } }));
                },
                openTitanChat() {
                    if (this.currentView !== 'conversation-messages') this.startNewConversation();
                    else this.$nextTick(() => this.$refs.message?.focus());
                },
                getTitanSettingStatus(setting) {
                    const statuses = {
                        'ai-providers': window.TitanWorkCore?.vault ? '{{ __('Vault ready') }}' : '{{ __('Configure') }}',
                        'offline-sync': this.titanConnectionLabel,
                        'workcore': window.TitanWorkCore ? '{{ __('Connected') }}' : '{{ __('Not connected') }}',
                        'device-security': window.TitanWorkCore?.vault ? '{{ __('Protected') }}' : '{{ __('Review') }}',
                    };
                    return statuses[setting] || '';
                },
                ...
                async getSession() {
                    ...
                    const chatbotId = this.activeChatbot?.id ?? @json(isset($chatbot) ? $chatbot->id : null);
                    try {
                        const localConversations = await window.TitanChatbotFrontendLocal?.listConversations(chatbotId, []);
                        if (localConversations?.length) {
                            this.conversations = localConversations.map(conversation => this.normalizeConversation(conversation));
                        }
                    } catch (error) {
                        console.warn('Local conversation hydration failed.', error);
                    }

                    let res;
                    let data;
                    try {
                        res = await fetch('{{ isset($routes) ? $routes['getSession'] : '' }}');
                        data = await res.json();
                    } catch (error) {
                        this.fetching = false;
                        this.setWidgetStatus({ type: 'warning', message: '{{ __('Offline — showing saved conversations.') }}' });
                        return;
                    }
                    ...
                    await window.TitanChatbotFrontendLocal?.cacheConversations(conversations, data.data?.id ?? chatbotId);
                    const mergedConversations = await window.TitanChatbotFrontendLocal?.listConversations(data.data?.id ?? chatbotId, conversations) || conversations;
                    this.conversations = mergedConversations.map(conversation => this.normalizeConversation(conversation));
                    ...
                    if (this.conversations.length) {
                        @if (true && isset($chatbot) && isset($session))
                            await this.initAbly();
                        @endif
                    }
                ...
                async startNewConversation() {
                    ...
                    @else
                        ...
                        let data;
                        try {
                            res = await fetch(`{{ isset($routes) ? $routes['conversations'] : '' }}`, {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json'
                                },
                            ...
                            const localConversation = await window.TitanChatbotFrontendLocal?.createConversationOffline({
                                chatbot_id: this.activeChatbot?.id,
                                prompt: this.activeChatbot?.bubble_message,
                                channel: 'web'
                            });
                            if (localConversation) {
                                this.conversations.push(this.normalizeConversation(localConversation));
                                this.activeConversation = localConversation.local_uuid;
                                this.activeConversationData = localConversation;
                                this.messages = [];
                                this.toggleView('conversation-messages');
                                this.setWidgetStatus({ type: 'warning', message: '{{ __('Conversation saved on this device and queued for sync.') }}' });
                            }
                            this.fetching = false;
                            return;
                        }
                        ...
                        await window.TitanChatbotFrontendLocal?.ensureConversation(data.data, this.activeChatbot?.id);
                ...
                async openConversation(conversationId) {
                    ...
                    @else
                        ...
                        try {
                            const localConversation = await window.TitanChatbotFrontendLocal?.ensureConversation(this.activeConversationData, this.activeChatbot?.id);
                            if (localConversation) this.activeConversationData.local_uuid = localConversation.local_uuid;
                            const draft = await window.TitanChatbotRepositories?.drafts.findForConversation(localConversation?.local_uuid || this.activeConversation);
                            await this.$nextTick();
                            if (draft?.payload?.body && this.$refs.message && !this.$refs.message.value) {
                                this.$refs.message.value = draft.payload.body;
                                this.$refs.sendBtn?.classList.toggle('active', true);
                            }
                        } catch (error) {
                            console.warn('Draft restoration failed.', error);
                        }
                        ...
                        try {
                            const localMessages = await window.TitanChatbotFrontendLocal?.listMessages(conversationId);
                            ...
                            console.warn('Local message hydration failed.', error);
                        }

                        let res;
                        let data;
                        try {
                            res = await fetch(`{{ isset($routes) ? $routes['conversations'] : '' }}/${conversationId}/messages`, {
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
                                    'X-timezone': Intl.DateTimeFormat().resolvedOptions().timeZone
                            ...
                            this.fetching = false;
                            this.setWidgetStatus({ type: 'warning', message: '{{ __('Offline — showing messages saved on this device.') }}' });
                            return;
                        }
                        ...
                        await window.TitanChatbotFrontendLocal?.cacheMessages(conversationId, serverMessages);
                        this.messages = await window.TitanChatbotFrontendLocal?.listMessages(conversationId) || serverMessages;
                ...
                async onSendMessage() {
                    ...
                    const editingLocalUuid = this.$refs.message.dataset.editingLocalUuid || null;
                    ...
                    @else
                        ...
                        try {
                            if (editingLocalUuid) {
                                const edited = await window.TitanChatbotLocalServices?.editQueuedMessage(editingLocalUuid, { message: messageString, payload: { message: messageString } });
                                newUserMessage = window.TitanChatbotFrontendLocal?.messagePayload(edited);
                                const existingIndex = this.messages.findIndex(message => message.local_uuid === editingLocalUuid);
                                if (existingIndex >= 0) this.messages.splice(existingIndex, 1);
                                delete this.$refs.message.dataset.editingLocalUuid;
                            } else {
                                newUserMessage = await window.TitanChatbotFrontendLocal?.sendMessageOffline(this.activeConversation, {
                                chatbot_id: this.activeChatbot?.id,
                                message: messageString,
                                file: pendingFile
                                });
                        ...
                        let res;
                        try {
                            res = await fetch(`{{ isset($routes) ? $routes['conversations'] : '' }}/${this.activeConversation}/messages`,
                                fetchOptions);
                        } catch (error) {
                            if (loaderMessage) this.messages = this.messages.filter(msg => msg.id !== loaderMessage.id);
                            newUserMessage.sync_status = 'queued';
                            await window.TitanChatbotFrontendLocal?.markFailed(newUserMessage.local_uuid, error);
                            newUserMessage.sync_status = 'failed';
                            this.setWidgetStatus({ type: 'warning', message: '{{ __('Message saved on this device. Retry when connected.') }}' });
                            this.scrollMessagesToBottom();
                            return;
                        }
                        ...
                        if (!res.ok) {
                            ...
                            await window.TitanChatbotFrontendLocal?.markFailed(newUserMessage.local_uuid, errorData.message || `HTTP ${res.status}`);
                            newUserMessage.sync_status = res.status === 409 || res.status === 412 ? 'conflict' : 'failed';
                        ...
                        const acknowledged = await window.TitanChatbotFrontendLocal?.acknowledgeMessage(newUserMessage.local_uuid, data?.user_message || {});
                        if (acknowledged) Object.assign(newUserMessage, acknowledged);
                ...
                renderGenerativeUiMessage(message, el, options = {}) {
                    if (!message?.ui_spec || !el || !window.TitanGenerativeUI) return null;
                    el.__titanGenerativeUi?.destroy?.();
                    const conversationId = message.conversation_id || this.activeConversation || 'preview';
                    const messageId = message.id || message.message_id || `${message.created_at || 'draft'}:${message.role || 'assistant'}`;
                    const suffix = options.persistenceSuffix || '';
                    const persistenceKey = `titan-chatbot-ui:${conversationId}:${messageId}${suffix}`;
                    const runtime = window.TitanGenerativeUI.renderSpec(message.ui_spec, el, {
                        surface: options.surface || 'chat',
                        persistenceKey,
                        storage: window.TitanGenerativeUiStorage || window.localStorage,
                        env: {
                            surface: options.surface || 'chat',
                        ...
                        onRuntimeError: detail => console.warn('Titan generative UI runtime warning', detail),
                    });
                    el.__titanGenerativeUi = runtime;
                    return runtime;
                },
                ...
                get titanViewLabel() {
                    const item = this.titanNavigation?.primary?.find(entry => entry.id === this.titanView);
                    return item?.label || this.getViewLabel() || this.titanNavigation?.name || '';
                },
                get titanConnectionLabel() {
                    if (this.titanConnectionState === 'offline') return '{{ __('Offline') }}';
                    if (this.titanConnectionState === 'syncing') return '{{ __('Syncing') }}';
                    return '{{ __('Online') }}';
                },
                get titanChatPlaceholder() {
                    const label = this.titanViewLabel?.toLowerCase?.() || '{{ __('this page') }}';
                    return `{{ __('Ask Titan about') }} ${label}…`;
                },
                getViewLabel() {
                    let viewName;
