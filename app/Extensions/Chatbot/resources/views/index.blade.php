        ...
        (() => {
            document.addEventListener('alpine:init', () => {
                Alpine.data('externalChatbotEditor', () => ({
                    ...
                    titanTemplates: @json(\App\Extensions\Chatbot\System\Titan\TitanRegistry::toArray()),
                    selectedTitanTemplate: null,
                    ...
                    applyTitanTemplate(template) {
                        this.selectedTitanTemplate = template.slug;
                        const chatbot = template.chatbot || {};
                        this.activeChatbot.title = chatbot.name || template.name;
                        this.activeChatbot.instructions = chatbot.system_prompt || '';
                        this.activeChatbot.suggested_prompts = (chatbot.suggested_prompts || []).map((prompt, index) => ({
                            name: `Prompt ${index + 1}`,
                            prompt,
                        }));
                        this.activeChatbot.suggested_prompts_enabled = this.activeChatbot.suggested_prompts.length > 0;
                        this.activeChatbot.color = template.color || this.activeChatbot.color;
                        this.activeChatbot.titan_template = template.slug;
                        this.externalChatbot && this.externalChatbot.toggleWindowState('open');
                    },
                    createNewChatObj() {
                    ...
                    async setEditingStep(step, skipCRUD = false) {
                        ...
                        if (step === '>') {
                            editingStep = Math.min(5, this.editingStep + 1);
                        } else if (step === '<') {
                        ...
                        if (
                            !skipCRUD &&
                            prevStep !== editingStep &&
                            (prevStep === 4 || prevStep === 3 || prevStep === 1) &&
                            this.activeChatbot.id !== 'new_chatbot'
                        ) {
