...
class ChatbotApplicationController extends Controller
{
    ...
    public function __construct(
        ...
        public GenerativeUiResponseComposer $generativeUi,
    ) {
    ...
    public function storeMessage(ChatbotHistoryStoreRequest $request, Chatbot $chatbot, string $sessionId, ChatbotConversation $chatbotConversation): ChatbotHistoryResource
    {
        ...
        $uiRequested = $this->generativeUi->shouldGenerate($prompt);
        $generationPrompt = $uiRequested ? $this->generativeUi->augmentPrompt($prompt, 'chat') : $prompt;
        ...
        $uiComposition = $uiRequested
            ? $this->generativeUi->compose($messageToUser, 'chat')
            : ['accepted' => false, 'fallback_text' => $messageToUser, 'spec' => null, 'changes' => [], 'issues' => []];

        $messageType = $uiComposition['accepted'] ? 'generative_ui' : 'text';
        $contentType = $uiComposition['accepted'] ? 'titan-ui' : 'text';
        $storedMessage = $uiComposition['accepted']
            ? $this->generativeUi->encodeEnvelope($uiComposition['fallback_text'], $uiComposition['spec'])
            : $messageToUser;
        ...
        $isHTML = ! $uiComposition['accepted'] && str_contains($response, 'lqd-ext-chatbot-html-response');
        ...
        return ChatbotHistoryResource::make($message)->additional([
            ...
            'isHTML'                              => $isHTML,
            'generative_ui_requested'              => $uiRequested,
            'generative_ui_accepted'               => $uiComposition['accepted'],
        ]);
    }
    ...
    public function appendMessage(ChatbotHistoryStoreRequest $request, Chatbot $chatbot, string $sessionId, ChatbotConversation $chatbotConversation)
    {
        ...
        ChatbotHistory::query()->create([
            ...
            'model'           => $this->setting->openai_default_model ?: ($chatbot->getAttribute('ai_model') ?: 'unknown'),
