@php
    $skillCreator = \App\Extensions\SystemAIChatSkills\System\Models\Skill::query()->where('source', 'seeded')->where('slug', 'skill-creator')->active()->first();
@endphp
                        ...
                        <div class="flex grow items-center gap-4 overflow-x-auto max-md:w-full">
                            ...
                            >
                                <button
                                    ...
                                    x-show="(!isGuest || !['personal', 'company'].includes(tab.key)) && (tab.key !== 'company' || company.id)"
                                    type="button"
                                >
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <div class="flex shrink-0 items-center gap-2">
                                            <template x-if="company.is_manager && company.id && !skill.is_company_installed">
                                                <button
                                                    class="rounded-full border border-foreground/10 px-3 py-2 text-3xs font-semibold hover:bg-foreground/5"
                                                    type="button"
                                                    @click.prevent="installSkillForCompany(skill)"
                                                >
                                                    {{ __('Install for Company') }}
                                                </button>
                                            </template>
                                            <template x-if="company.is_manager && company.id && skill.is_company_installed">
                                                <button
                                                    class="rounded-full border border-foreground/10 px-3 py-2 text-3xs font-semibold text-red-500 hover:bg-red-500/5"
                                                    type="button"
                                                    @click.prevent="removeSkillFromCompany(skill)"
                                                >
                    ...
                    <div class="lqd-modal-head flex items-center gap-3 border-b border-foreground/5 pb-3 max-md:flex-wrap">
                        <div class="flex items-center gap-2.5 max-md:w-full max-md:flex-wrap">
                            ...
                            <span
                                ...
                                x-text="skillScopeLabel(detailSkill)"
                            ></span>
                            <span
                                class="whitespace-nowrap rounded-full bg-foreground/5 px-2 py-1 text-[11px] font-semibold"
                                x-show="detailSkill?.version"
                                x-text="'v' + detailSkill.version"
                            ></span>
                        </div>
                    ...
                    <div class="flex min-h-0 flex-1 pt-3">
                        ...
                        <template x-if="!detailLoading && detailSkill">
                            <div class="flex min-h-0 flex-1 gap-3 max-md:flex-wrap">
                                {{-- File Tree Sidebar --}}
                                <div class="w-52 shrink-0 overflow-y-auto px-4 py-2 max-md:max-h-56 max-md:w-full">
                                    <div class="mb-4 border-b border-foreground/5 pb-3" x-show="!isGuest">
                                        <div class="mb-2 flex items-center justify-between gap-2">
                                            <span class="text-[11px] font-semibold uppercase tracking-wide text-foreground/50">{{ __('Versions') }}</span>
                                            <x-tabler-loader-2 class="size-3 animate-spin" x-show="versionLoading" />
                                        </div>
                                        <template x-if="!versionLoading && versionHistory.length === 0">
                                            <p class="m-0 text-[11px] text-foreground/40">{{ __('No version history available.') }}</p>
                                        </template>
                                        <div class="space-y-2" x-show="versionHistory.length > 0">
                                            <template x-for="release in versionHistory" :key="release.id">
                                                <div class="rounded-lg border border-foreground/5 p-2">
                                                    <div class="flex items-center justify-between gap-2">
                                                        <span class="text-[11px] font-semibold" x-text="'v' + release.version"></span>
                                                        <span class="text-[9px] uppercase opacity-50" x-text="release.lifecycle_status"></span>
                                                    </div>
                                                    <div class="mt-1 flex items-center justify-between gap-2">
                                                        <span class="text-[9px] uppercase opacity-40">{{ __('Integrity') }}</span>
                                                        <span
                                                            class="rounded-full bg-foreground/5 px-1.5 py-0.5 text-[9px] font-semibold uppercase"
                                                            x-text="release.verification_status || 'unverified'"
                                                        ></span>
                                                    </div>
                                                    <div class="mt-2 flex flex-wrap gap-1" x-show="detailSkill?.is_owner || detailSkill?.is_added || detailSkill?.is_company_installed">
                                                        <button
                                                            class="rounded border border-foreground/10 px-1.5 py-1 text-[9px] hover:bg-foreground/5"
                                                            type="button"
                                                            x-show="versionCanManage && ['published', 'deprecated'].includes(release.lifecycle_status) && release.id !== currentVersionId"
                                                            @click="rollbackVersion(release)"
                                                        >{{ __('Rollback') }}</button>
                                                        <button
                                                            class="rounded border border-foreground/10 px-1.5 py-1 text-[9px] hover:bg-foreground/5"
                                                            type="button"
                                                            x-show="versionCanManage && ['draft', 'approved'].includes(release.lifecycle_status)"
                                                            @click="publishVersion(release)"
                                                        >{{ __('Publish') }}</button>
                                                        <button
                                                            class="rounded border border-foreground/10 px-1.5 py-1 text-[9px] hover:bg-foreground/5"
                                                            type="button"
                                                            x-show="versionCanManage"
                                                            @click="verifyVersion(release)"
                                                        >{{ __('Verify') }}</button>
                                                        <button
                                                            class="rounded border border-foreground/10 px-1.5 py-1 text-[9px] hover:bg-foreground/5"
                                                            type="button"
                                                            x-show="detailSkill?.is_added && ['published', 'deprecated'].includes(release.lifecycle_status)"
                                                            @click="pinPersonalVersion(release)"
                                                        >{{ __('Pin personal') }}</button>
                                                        <button
                                                            class="rounded border border-foreground/10 px-1.5 py-1 text-[9px] hover:bg-foreground/5"
                                                            type="button"
                                                            x-show="company.is_manager && detailSkill?.is_company_installed && ['published', 'deprecated'].includes(release.lifecycle_status)"
                                                            @click="pinCompanyVersion(release)"
                                                        >{{ __('Pin company') }}</button>
                                                    </div>
                                                </div>
                                            </template>
                                        </div>
                                ...
                                <div
                                    ...
                                    <div class="mt-5" x-show="skillTools(detailSkill).length > 0">
                                        <div class="mb-2 flex items-center justify-between gap-2">
                                            <h3 class="m-0 text-xs font-semibold uppercase tracking-wide text-foreground/50">{{ __('Tools') }}</h3>
                                            <span class="text-[10px] text-foreground/40" x-text="skillTools(detailSkill).length + ' {{ __('declared') }}'"></span>
                                        </div>
                                        <div class="grid gap-2 md:grid-cols-2">
                                            <template x-for="tool in skillTools(detailSkill)" :key="tool.key">
                                                <div class="rounded-xl border border-foreground/5 bg-foreground/[2%] p-3">
                                                    <div class="flex items-start justify-between gap-2">
                                                        <div class="min-w-0">
                                                            <p class="m-0 truncate text-xs font-semibold" x-text="tool.key"></p>
                                                            <p class="m-0 mt-1 line-clamp-2 text-[11px] text-foreground/50" x-text="tool.description || tool.function_name || ''"></p>
                                                        </div>
                                                        <span
                                                            class="shrink-0 rounded-full px-2 py-1 text-[9px] font-semibold"
                                                            :class="tool.executable ? 'bg-green-500/10 text-green-600' : 'bg-amber-500/10 text-amber-600'"
                                                            x-text="tool.executable ? '{{ __('Available') }}' : '{{ __('Adapter required') }}'"
                                                        ></span>
                                                    </div>
                                                    <div class="mt-2 flex flex-wrap gap-1.5">
                                                        <span class="rounded bg-foreground/5 px-1.5 py-1 text-[9px]" x-text="'{{ __('Approval') }}: ' + tool.approval"></span>
                                                        <span class="rounded bg-primary/10 px-1.5 py-1 text-[9px] text-primary" x-show="tool.required">{{ __('Required') }}</span>
                                                        <span class="rounded bg-foreground/5 px-1.5 py-1 text-[9px]" x-text="tool.source || 'custom'"></span>
                                                    </div>
                                                </div>
                                            </template>
                                        </div>
            ...
            >
                ...
                >
                    ...
                    <div
                        ...
                        <div
                            class="mb-5 rounded-xl border border-foreground/5 p-4"
                            x-show="company.is_manager && company.id"
                            x-cloak
                        >
                            <p class="mb-3 text-2xs font-semibold text-heading-foreground">{{ __('Create skill for') }}</p>
                            <div class="flex gap-2">
                                <button
                                    class="rounded-full border px-3 py-2 text-2xs"
                                    :class="uploadOwnership === 'user' ? 'border-primary text-primary' : 'border-foreground/10'"
                                    type="button"
                                    @click="uploadOwnership = 'user'"
                                >{{ __('My library') }}</button>
                                <button
                                    class="rounded-full border px-3 py-2 text-2xs"
                                    :class="uploadOwnership === 'company' ? 'border-primary text-primary' : 'border-foreground/10'"
                                    type="button"
                                    @click="uploadOwnership = 'company'"
                                >{{ __('Company library') }}</button>
                            </div>
                            <label class="mt-4 block text-2xs" x-show="uploadOwnership === 'company'">
                                <span class="mb-2 block font-medium">{{ __('Company visibility') }}</span>
                                <select class="h-10 w-full rounded-lg border border-foreground/10 px-3" x-model="uploadVisibility">
                                    <option value="company_private">{{ __('Managers only') }}</option>
                                    <option value="company_shared">{{ __('Company members') }}</option>
                                </select>
                            </label>
                        </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('skillsModal', () => ({
                ...
                uploadOwnership: 'user',
                uploadVisibility: 'company_private',
                skillIdMap: {},

                async init() {
                    await this.fetchSkills();
                    if (!this.isGuest) await this.fetchToolCatalog();
                ...
                async fetchSkills() {
                    ...
                    try {
                        ...
                        if (!this.company?.id && this.activeTab === 'company') this.activeTab = 'all';
                    ...
                    if (skill._fromPreview && skill._files?.length) {
                        try {
                            const res = await fetch('/dashboard/user/skills/create-from-content', {
                                ...
                                body: JSON.stringify({
                                    files: skill._files,
                                    ownership_type: this.uploadOwnership,
                                    visibility: this.uploadOwnership === 'company' ? this.uploadVisibility : undefined
                                }),
                            });
                            const data = await res.json();
                            if (res.ok) {
                ...
                async installSkillForCompany(skill) {
                    if (!this.company.is_manager) return;
                    try {
                        const res = await fetch(`/dashboard/user/skills/company/install/${skill.id}`, {
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content,
                                'X-Requested-With': 'XMLHttpRequest'
                        ...
                        const data = await res.json();
                        if (!res.ok) throw new Error(data.message || '{{ __('Company installation failed.') }}');
                        skill.is_company_installed = true;
                        skill.company_enabled = true;
                        toastr.success(data.message);
                        await this.fetchSkills();
                    } catch (err) {
                        toastr.error(err.message || '{{ __('Company installation failed.') }}');
                    }
                },

                async removeSkillFromCompany(skill) {
                    if (!this.company.is_manager) return;
                    try {
                        const res = await fetch(`/dashboard/user/skills/company/remove/${skill.id}`, {
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content,
                                'X-Requested-With': 'XMLHttpRequest'
                        ...
                        const data = await res.json();
                        if (!res.ok) throw new Error(data.message || '{{ __('Company removal failed.') }}');
                        skill.is_company_installed = false;
                        toastr.success(data.message);
                        await this.fetchSkills();
                    } catch (err) {
                        toastr.error(err.message || '{{ __('Company removal failed.') }}');
                    }
                },

                async configureSkillForCompany(skill, settings) {
                    if (!this.company.is_manager) return;
                    const res = await fetch(`/dashboard/user/skills/company/configure/${skill.id}`, {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content,
                    ...
                    const data = await res.json();
                    if (!res.ok) throw new Error(data.message || '{{ __('Company settings update failed.') }}');
                    return data;
                },

                skillScopeLabel(skill) {
                    if (!skill) return '';
                    if (skill.ownership_type === 'company') {
                        return skill.visibility === 'company_shared'
                            ? '{{ __('Company Shared Skill') }}'
                            : '{{ __('Company Private Skill') }}';
                    }
                    if (skill.ownership_type === 'platform' || skill.is_public) return '{{ __('Public Skill') }}';
                    return '{{ __('Personal Skill') }}';
                },
                ...
                async fetchToolCatalog() {
                    try {
                        const res = await fetch('/dashboard/user/skills/tool-catalog', {
                            headers: {
                                'X-Requested-With': 'XMLHttpRequest',
                        ...
                        if (!res.ok) return;
                        const data = await res.json();
                        this.toolCatalog = Object.fromEntries(
                            (data.tools || []).map(tool => [tool.key, tool])
                        );
                    } catch (err) {
                        console.error('Failed to load skill tool catalog:', err);
                    }
                },

                skillTools(skill) {
                    const contract = skill?.skill_contract || skill?.metadata?.skill_contract || {};
                    const toolSet = contract?.tools || {};
                    const allowed = Array.isArray(toolSet) ? toolSet : (toolSet.allowed || []);
                    const required = Array.isArray(toolSet?.required) ? toolSet.required : [];

                    return allowed
                        .map(item => typeof item === 'string' ? { key: item } : item)
                        .filter(item => item && item.key)
                        .map(item => {
                            const runtime = this.toolCatalog[item.key] || {};
                            return {
                                ...runtime,
                                ...item,
                                key: item.key,
                                approval: item.approval || runtime.approval || 'never',
                                executable: Boolean(runtime.executable),
                                required: required.includes(item.key),
                                source: item.source || runtime.source || 'custom'
                            };
                        });
                },
                ...
                async viewSkill(skill) {
                    ...
                    this.detailActiveFile = 'SKILL.md';
                    this.versionHistory = [];
                    this.currentVersionId = skill.current_version_id || null;
                    this.latestPublishedVersionId = skill.latest_published_version_id || null;
                    this.versionCanManage = false;
                    ...
                    try {
                        ...
                        if (data.skill) {
                            ...
                            this.currentVersionId = data.skill.current_version_id || null;
                            this.latestPublishedVersionId = data.skill.latest_published_version_id || null;
                            if (!this.isGuest) await this.fetchVersions(data.skill.id);
                        }
                    } catch (err) {
                ...
                async fetchVersions(skillId) {
                    this.versionLoading = true;
                    try {
                        const res = await fetch(`/dashboard/user/skills/${skillId}/versions`, {
                            headers: { 'X-Requested-With': 'XMLHttpRequest' }
                        });
                        if (!res.ok) return;
                        const data = await res.json();
                        this.versionHistory = data.versions || [];
                        this.currentVersionId = data.current_version_id || this.currentVersionId;
                        this.latestPublishedVersionId = data.latest_published_version_id || this.latestPublishedVersionId;
                        this.versionCanManage = Boolean(data.can_manage);
                    } catch (err) {
                ...
                async rollbackVersion(release) {
                    if (!this.detailSkill?.id) return;
                    const res = await fetch(`/dashboard/user/skills/${this.detailSkill.id}/versions/${release.id}/rollback`, {
                        method: 'POST',
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content,
                    ...
                    const data = await res.json();
                    if (!res.ok) return toastr.error(data.message || '{{ __('Rollback failed.') }}');
                    toastr.success(data.message);
                    await this.viewSkill(this.detailSkill);
                    await this.fetchSkills();
                },

                async publishVersion(release) {
                    if (!this.detailSkill?.id) return;
                    const res = await fetch(`/dashboard/user/skills/${this.detailSkill.id}/versions/${release.id}/publish`, {
                        method: 'POST',
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content,
                    ...
                    const data = await res.json();
                    if (!res.ok) return toastr.error(data.message || '{{ __('Publish failed.') }}');
                    toastr.success(data.message);
                    await this.viewSkill(this.detailSkill);
                    await this.fetchSkills();
                },

                async verifyVersion(release) {
                    if (!this.detailSkill?.id) return;
                    const res = await fetch(`/dashboard/user/skills/${this.detailSkill.id}/versions/${release.id}/verify`, {
                        method: 'POST',
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content,
                    ...
                    const data = await res.json();
                    if (!res.ok) {
                        await this.fetchVersions(this.detailSkill.id);
                        return toastr.error(data.message || '{{ __('Package verification found differences.') }}');
                    }
                    toastr.success(data.message);
                    await this.fetchVersions(this.detailSkill.id);
                },

                async pinPersonalVersion(release) {
                    const res = await fetch(`/dashboard/user/skills/configure/${this.detailSkill.id}`, {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-Requested-With': 'XMLHttpRequest',
                        ...
                        body: JSON.stringify({ skill_version_id: release.id, update_policy: 'pinned' })
                    });
                    const data = await res.json();
                    if (!res.ok) return toastr.error(data.message || '{{ __('Version pin failed.') }}');
                    toastr.success(data.message);
                    this.detailSkill.installed_version_id = release.id;
                    this.detailSkill.update_policy = 'pinned';
                },

                async pinCompanyVersion(release) {
                    const res = await fetch(`/dashboard/user/skills/company/configure/${this.detailSkill.id}`, {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-Requested-With': 'XMLHttpRequest',
                    ...
                    const data = await res.json();
                    if (!res.ok) return toastr.error(data.message || '{{ __('Company version pin failed.') }}');
                    toastr.success(data.message);
                    this.detailSkill.company_version_id = release.id;
                    this.detailSkill.company_update_policy = 'pinned';
                },
                ...
                async submitUserUpload() {
                    ...
                    formData.append('ownership_type', this.uploadOwnership);
                    if (this.uploadOwnership === 'company') formData.append('visibility', this.uploadVisibility);
