<div
    ...
    <div
        class="relative cursor-pointer rounded-md px-3.5 py-2 text-2xs font-medium text-heading-foreground/60 transition-all hover:bg-heading-foreground/[3%] hover:text-heading-foreground [&_.lqd-shared-chat-modal-trigger]:absolute [&_.lqd-shared-chat-modal-trigger]:inset-0 [&_.lqd-shared-chat-modal-trigger]:z-2 [&_.lqd-shared-chat-modal-trigger]:opacity-0">
        <x-modal
        ...
        >
            <x-slot:trigger
                class="lqd-shared-chat-modal-trigger h-6 max-md:inline-flex max-md:size-8 max-md:items-center max-md:justify-center max-md:rounded-full max-md:bg-background max-md:p-0 max-md:text-foreground max-md:shadow-md max-md:hover:text-primary-foreground md:h-6 md:px-2 md:py-1 md:text-2xs"
                variant="primary"
                    ...
                    function instructionsManager(chatId) {
                        return {
                            ...
                            async loadInstructions() {
                                ...
                                try {
                                    const response = await fetch(`/system-ai-chat-memory/instructions?chat_id=${this.chatId}`, {
                                        headers: {
                            ...
                            async saveInstructions() {
                                ...
                                try {
                                    const response = await fetch('/system-ai-chat-memory/instructions', {
                                        method: 'POST',
                                        headers: {
