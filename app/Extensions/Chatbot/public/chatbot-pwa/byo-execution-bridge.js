(() => {
  ...
  function conversationMessages(message, messages = []) {
    ...
    if (!normalised.length || normalised[normalised.length - 1]?.content !== message) normalised.push({ role: 'user', content: redactText(message) });
    return normalised;
  }
  ...
  function shouldUseDeviceProvider() {
    ...
    return ['device-first', 'private-hybrid', 'cloud-preferred'].includes(mode) && Boolean(connectedProvider()[0]);
  }
