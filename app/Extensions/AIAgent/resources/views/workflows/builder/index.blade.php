            ...
            Alpine.data('workflowBuilder', (existingWorkflow, channels, memories, avatarPresets, initialCopilotMessages, templateData, connectors) => ({
                ...
                get availableVariables() {
                    ...
                    this.steps.forEach((step, idx) => {
                        ...
                        const s = step.config?.store_output_as;
                        if (s) {
                            const name = `{${s}}`;
                            if (!seen.has(name)) {
                                seen.add(name);
                                dynamic.push({
                                    name,
                                    description: `{{ trans('Step') }} ${stepNum} · ${typeLabel} {{ trans('output') }}`
                                });
                            }
                        }
                        if (step.type === 'path' && step.config?.paths) {
                            step.config.paths.forEach(path => {
                                (path.steps ?? []).forEach(ss => {
                                    const v = ss.config?.store_output_as;
                                    if (v) {
                                        const name = `{${v}}`;
                                        if (!seen.has(name)) {
                                            seen.add(name);
                                            dynamic.push({
                                                name,
                                                description: `{{ trans('Step') }} ${stepNum} · ${this.actionTypeLabel(ss.type)} {{ trans('output') }}`
                                            });
                                        }
                                    }
                                });
                ...
                stepSummary(step) {
                    ...
                    if (step.type === 'ai_call') return step.config?.user_message || step.config?.system_prompt?.substring(0, 50) ||
                        '{{ trans('No message set') }}';
                    if (step.type === 'send_message') return step.config?.message || '{{ trans('No message set') }}';
                ...
                stepHasMissingRequired(step) {
                    ...
                    const actionEventTypes = ['external_chatbot', 'marketing_bot', 'social_media_agent', 'gmail', 'outlook'];
                ...
                async saveWorkflow(silent = false) {
                    ...
                    try {
                        ...
                        if (data.status === 'success') {
                            if (!silent) toastr.success(data.message);
                ...
                applyCopilotMutation(toolCall) {
                    if (toolCall.name === 'add_step') {
                        ...
                        const outputTypes = ['ai_call', 'generate_report', 'external_chatbot', 'marketing_bot', 'social_media_agent', 'gmail', 'outlook'];
                        const config = {
                            ...(args.config ?? {})
                        };
