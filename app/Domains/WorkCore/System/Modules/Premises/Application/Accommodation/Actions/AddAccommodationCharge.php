...
final class AddAccommodationCharge implements BusinessActionHandlerContract
{
    ...
    {
        $record = $this->repository->addCharge($this->required($request, 'reservation_public_id'), $request->payload, $request->companyId, $request->actorId);
        return new ActionHandlerResult(
            data: $record,
            aggregate: new TypedReference('accommodation_charge', (string) ($record['public_id'] ?? '')),
            events: [new PendingDomainEvent('workcore.accommodation.charge.added', 1, ['record' => $record])],
        );
    }
