...
final class CheckOutAccommodationStay implements BusinessActionHandlerContract
{
    ...
    {
        $record = $this->repository->checkOut($this->required($request, 'stay_public_id'), $request->payload, $request->companyId, $request->actorId);
        return new ActionHandlerResult(
            data: $record,
            aggregate: new TypedReference('accommodation_stay', (string) ($record['public_id'] ?? '')),
            events: [new PendingDomainEvent('workcore.accommodation.stay.checked_out', 1, ['record' => $record])],
        );
    }
