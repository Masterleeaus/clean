...
final class EndPremisesOccupancy implements BusinessActionHandlerContract
{
    public function __construct(private PropertyAccommodationRepositoryContract $repository) {}

    public function handle(ActionRequest $request): ActionHandlerResult
    {
        $record = $this->repository->endOccupancy($this->required($request, 'occupancy_public_id'), $request->payload, $request->companyId, $request->actorId);
        return new ActionHandlerResult(
            data: $record,
            aggregate: new TypedReference('premises_occupancy', (string) ($record['public_id'] ?? '')),
            events: [new PendingDomainEvent('workcore.premises.occupancy.ended', 1, ['record' => $record])],
        );
    }
