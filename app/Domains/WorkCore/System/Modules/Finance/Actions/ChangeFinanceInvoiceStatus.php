...
final class ChangeFinanceInvoiceStatus implements BusinessActionHandlerContract
{
    ...
    {
        $publicId = (string) ($request->payload['invoice_public_id'] ?? ''); $status = (string) ($request->payload['status'] ?? ''); if ($publicId === '' || $status === '') throw new InvalidArgumentException('Invoice and status are required.'); $record = $this->repository->transitionInvoice($publicId, $status, $request->payload, $request->companyId, $request->actorId);
