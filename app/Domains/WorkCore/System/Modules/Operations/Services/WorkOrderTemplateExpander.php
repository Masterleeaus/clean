...
final class WorkOrderTemplateExpander
{
    ...
    public function expand(int $companyId, int $workOrderId, int $actorId, array $serviceLine, int $sortOrder): void
    {
        $service = $this->db->table('tz_services')
            ->where('company_id', $companyId)
            ->where('public_id', (string) ($serviceLine['service_public_id'] ?? ''))
            ->whereNull('deleted_at')
            ->first(['id', 'code', 'name', 'description', 'default_duration_minutes', 'base_price']);
        if (! $service) {
    ...
    private function resolveTemplate(int $companyId, int $serviceId, mixed $templatePublicId): ?object
    {
        ...
        if (is_string($templatePublicId) && trim($templatePublicId) !== '') {
            $template = $query->where('public_id', $templatePublicId)->first(['id', 'estimated_duration_minutes']);
            if (! $template) {
        ...
        return $query->where('status', 'active')->orderBy('id')->first(['id', 'estimated_duration_minutes']);
    }
}
