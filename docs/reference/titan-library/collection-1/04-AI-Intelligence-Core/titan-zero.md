...
## 24. Vertical Context Loading

Titan Zero becomes a **vertical specialist** when a job context is loaded. This is the mechanism that turns a generic assistant into an industry expert.

### How it works

At job creation, the vertical overlay activates and Titan Zero loads a vertical-specific context pack:

```php
$overlay  = VerticalOverlay::load($job->vertical);
$context  = TitanZeroContextLoader::loadForJob($job);
// context includes: knowledge pack, terminology, compliance rules,
//                   checklist intelligence, artefact guides, pricing logic
```

### Vertical knowledge pack contents

Each vertical has a structured knowledge pack in `resources/ai/verticals/{slug}/`:

- `context.json` — primary context loaded into Titan Zero at job start
- `standards/` — industry manuals, compliance documents, inspection standards
- `checklists/` — reference checklists and common failure points
- `compliance/` — mandatory reporting triggers, regulatory requirements
- `artefacts/` — document templates and guide for auto-fill

### AI behaviours activated per vertical

- Quote generation using vertical-specific pricing logic
- Compliance warning generation (e.g., "SWMS not signed — cannot proceed")
- Artefact pre-fill from job conversation and photo data
- Escalation detection tuned to vertical (e.g., NDIS incident → mandatory notification)
- Objection handling trained on vertical customer concerns

See `docs/vertical-ai-training-architecture.md` for the full vertical-by-vertical knowledge pack specification.

---

## 25. BYO API Key — Zero Philosophy Alignment

Titan Zero implements the **Zero vendor lock-in** principle from the Zero Philosophy:

- Companies supply their own AI API key (OpenAI, Anthropic, Gemini)
- No AI markup or platform surcharge on usage
- Provider abstraction layer allows switching provider without changing any caller
- Company data never trains third-party models
- All AI calls are tenant-scoped and auditable

The BYO key is stored per-company, encrypted at rest, and injected into Titan Core at execution time. Titan Zero's policy and context layers are entirely provider-agnostic.

See `docs/philosophy/00-zero-philosophy.md` for the full Zero doctrine.

---

## 26. Node-Specific AI Contexts

Titan Zero operates in different modes depending on which node surfaces it:

| Node | Titan Zero Context Mode |
|---|---|
| **Titan Go** | Field guidance — checklist hints, artefact pre-fill, compliance gate warnings |
| **Titan Zero (node)** | Full consultation mode — reports, compliance advice, document generation |
| **Titan Hello** | Conversation intake — intent detection, booking extraction, lead qualification |
| **Ground Zero** | Dispatch intelligence — anomaly detection, SLA breach risk, re-routing suggestions |
| **Titan Solo** | Simplified mode — next action, quote draft, cashflow summary |
| **Titan Studio** | Content generation — ads, emails, social posts, vertical copy |
| **ZeroPay** | Financial analysis — cashflow forecast, late payment drafts, BAS summary |
| **Titan Pro** | Owner insights — performance trends, crew anomalies, strategic recommendations |
| **Zero Fuss** | Customer-facing summaries — plain language job summaries, artefact explanations |

Context mode is set at the node surface level and restricts both available tools and response style.
