 ...
 */
function initChatApp(user, pusherKey, pusherCluster, companyId) {
    ChatApp.currentUser = user;
    ChatApp.currentCompanyId = Number(companyId);
    if (!Number.isInteger(ChatApp.currentCompanyId) || ChatApp.currentCompanyId < 1) {
        throw new Error('A valid active company is required to start chat.');
    }
...
function initializePusher() {
    ...
    window.Echo.join(`online.${ChatApp.currentCompanyId}`)
        .here((users) => {
    ...
    if (ChatApp.currentUser && ChatApp.currentUser.id) {
        window.Echo.private(`App.Models.User.${ChatApp.currentUser.id}.${ChatApp.currentCompanyId}`)
            .listen('ConversationCreated', (e) => {
                if (Number(e.company_id) !== ChatApp.currentCompanyId) {
                    return;
                }
...
function renderConversations(convs) {
    ...
    convs.forEach(conv => {
        ...
        const lastMessageText = lastMessage
            ? (lastMessage.attachment_url ? 'Sent an attachment' : lastMessage.message)
            : 'No messages yet';
...
function appendMessage(message) {
    ...
    if (isSent) {
        ...
        const hasOnlyMedia = message.attachment_url && (!message.message || message.message.trim() === '');
    ...
    if (ChatApp.currentConversationId && (message.attachment_url || (message.message && message.message.match(/(https?:\/\/[^\s]+)/g)))) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (chatItem.length > 0) {
                const messageText = data.message || (data.attachment_url ? 'Sent an attachment' : '');
        ...
        window.Echo.private(channelName)
            .listen('MessageSent', (e) => {
                ...
                if (!ChatApp.currentConversationId || parseInt(ChatApp.currentConversationId) !== parseInt(e.conversation_id)) {
                    ...
                    const messageText = e.message || (e.attachment_url ? 'Sent an attachment' : '');
                    chatItem.find('p').first().text(messageText);
